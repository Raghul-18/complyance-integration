...
1. Was the **same `Idempotency-Key`** sent on both attempts, or was a new
   key generated for the second attempt?
...
3. What was the **exact timeout** the customer observed: a client side
   ...
   arrived late or was itself dropped?
4. Was the **request body identical** between the two attempts, byte for
   byte, or did the ERP recompute `totals` before resubmitting? For
   example, did it apply a rounding fix, a tax rate correction, or repull
   data from source in between?
5. What were the **two document IDs** and the **two HTTP status codes**
   received?
6. Did the tax mismatch happen on the second submission, or on a later
   retry?
7. What time gap was there between the two submissions? A near
   simultaneous gap (sub second, suggesting a race) versus minutes apart
   (suggesting a deliberate manual retry after the customer noticed the
   timeout) point at different causes.
8. Is there a **retry or queue layer on the ERP side** sitting between `desert-star-erp` and our API that could
   itself be generating the duplicate, independent of anything our API
   did?
9. Were both requests sent to the **same environment and base URL**, and
   by the same client or service account? (Ruling out a "one hit staging,
   one hit local" type mix up.)
10. Does the customer have **their own request and response logs** for
    both attempts (timestamps, headers, status codes, response bodies)?
    Without those, we are limited to what our own audit log captured.
...
1. Submit `samples/valid-invoice.json` (invoice `INV-2025-1001`) with a
   fixed `Idempotency-Key`, for example `repro-1`.
2. Immediately submit the **exact same payload** again with the **same**
   ...
   - **Expected (current code):** the second response returns the *same*
     ...
     `INSERT`).
3. Submit a **modified** payload (for example a different `invoiceNo`, to
   simulate the ERP changing something before retrying) with the **same**
   `Idempotency-Key: repro-1`.
...
4. Submit the same payload **twice with no `Idempotency-Key` header at
   ...
   - **Expected (current code):** the same behavior as step 2, the same
...
- The `X-Correlation-ID` returned on both responses, if the customer's
  client captured response headers. This lets us pull the exact
  structured log lines for each request from application logs (never the
  raw payload, per the logging redaction rule, but method, path, status,
  latency, and the safe payload metadata are there).
- The two `documentId` values and their current `status` and `errors` via
...
- Whichever request produced "the tax total did not match": its exact
  ...
  submitted versus expected values in the error message.
...
2. **A retry or queue layer upstream of the ERP integration** generating a
...
3. **A pre fix check then insert race condition**, if this report describes
   ...
   used to confirm or rule this out directly, since it is covered by an
   automated test that currently passes.
4. **The "tax total did not match" failure is unrelated to the duplicate
   documentId issue**, meaning two separate problems bundled into one
...
## Immediate containment and workaround

- Ask the customer to **always send an explicit `Idempotency-Key`** header
  derived from something stable on their side (for example their own
  internal transaction ID), rather than relying on the deterministic
...
- If the two documents are confirmed duplicates of a single real invoice,
  ...
  There is no `DELETE` or cancel endpoint in this prototype; this is a
...
- **Already implemented in this codebase**, worth explicitly verifying
  ...
  constraint on `idempotency_key`, not an application level check then
  insert. If the customer's report is against an older or different
  implementation, this is the concrete fix to point to.
- **Recommend the customer's ERP always sends an explicit
  `Idempotency-Key`** rather than relying on any server side fallback
  derived from mutable fields. This removes an entire class of "field
  changed between retries" ambiguity.
- **Add existence check tooling for the audit trail** so this kind of
  investigation does not depend on someone manually cross referencing two
  `documentId`s. For example a lookup by `invoice_no` (not just
  `document_id`) on `GET /api/v1/audit`, since the customer is unlikely to
  know a `documentId` when first reporting the issue.
- **Consider validating `amountDue`** in the same reconciliation pass as
  `netAmount`, `taxAmount`, and `grossAmount` (currently unvalidated, see
  `mapping.md`), if "tax total did not match" turns out to trace back to
  an inconsistency in a field that is not checked today.

## What to escalate, and to whom

- **To the Technical Implementation Consultant or the customer's
  integration team:** clarifying questions 1 through 4 and 8 through 10
  above. We cannot determine root cause without the customer's own
  request logs and confirmation of their retry behavior.
- **To Product:** whether the deterministic fallback idempotency key
  (used only when the customer does not send `Idempotency-Key`) should be
...
- **To Engineering:** only if reproduction step 2 (same key, same payload)
  ...
  indicate the `UNIQUE` constraint fix is not present or is not working in
